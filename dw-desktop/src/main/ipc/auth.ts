...
import { saveApiKey, saveOAuthCredentials } from '../credentials'
import { getEnvs } from '../config'
import type { ConnectionStatus, IPCResult, StoredEnv } from '../../shared/types'
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        if (!loginResponse.ok) {
          return { ok: false, error: `Login failed with status ${loginResponse.status}` }
        }
